...
const AdminConsole = (props: Props) => {
    ...
    useEffect(() => {
        ...
        props.actions.loadRolesIfNeeded([
            "channel_user",
            "team_user",
            "system_user",
            "channel_admin",
    ...
    const renderRoutes = (extraProps: ExtraProps) => {
        ...
        const schemas: AdminDefinitionSubSection[] = Object.values(
            adminDefinition,
        ).flatMap((section: AdminDefinitionSection) => {
            let isSectionHidden = false;
            if (typeof section.isHidden === "function") {
                isSectionHidden = section.isHidden(
                    config,
                    { search },
                    license,
                    buildEnterpriseReady,
        ...
        const schemaRoutes = schemas.map(
            (item: AdminDefinitionSubSection, index: number) => {
    ...
    const { license, config, environmentConfig, showNavigationPrompt, roles } =
        props;
    const {
    ...
    if (!props.currentUserHasAnAdminRole) {
        return <Redirect to={props.unauthorizedRoute} />;
    }
    ...
    if (
        ...
        return <div className="admin-console__wrapper admin-console" />;
    }
