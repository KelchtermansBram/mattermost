...
const BrowseChannels = makeAsyncComponent(
    "BrowseChannels",
    lazy(() => import("components/browse_channels")),
);
const EditCategoryModal = makeAsyncComponent(
    "EditCategoryModal",
    lazy(() => import("components/edit_category_modal")),
);
const CreateUserGroupsModal = makeAsyncComponent(
    "CreateUserGroupsModal",
    lazy(() => import("components/create_user_groups_modal")),
);
const InvitationModal = makeAsyncComponent(
    "InvitationModal",
    lazy(() => import("components/invitation_modal")),
);
const KeyboardShortcutsModal = makeAsyncComponent(
    "KeyboardShortcutsModal",
    lazy(
        () =>
            import("components/keyboard_shortcuts/keyboard_shortcuts_modal/keyboard_shortcuts_modal"),
    ),
);
const NewChannelModal = makeAsyncComponent(
    "NewChannelModal",
    lazy(() => import("components/new_channel_modal/new_channel_modal")),
);
const UserSettingsModal = makeAsyncComponent(
    "UserSettingsModal",
    lazy(() => import("components/user_settings/modal")),
);
...
export default class Sidebar extends React.PureComponent<Props, State> {
    ...
    componentWillUnmount() {
        window.removeEventListener(
            "click",
            this.handleClickClearChannelSelection,
        );
            ...
            if (isKeyPressed(event, Constants.KeyCodes.FORWARD_SLASH)) {
                ...
                if (this.props.isKeyBoardShortcutModalOpen) {
                    this.props.actions.closeModal(
                        ModalIdentifiers.KEYBOARD_SHORTCUTS_MODAL,
                    );
                } else {
                ...
                isKeyPressed(event, Constants.KeyCodes.A) &&
                event.shiftKey
            ) {
                event.preventDefault();
    ...
    renderModals = () => {
        ...
        return <>{moreDirectChannelsModal}</>;
    };

    closeEditRHS = () => {
        if (
            this.props.rhsOpen &&
            this.props.rhsState === RHSStates.EDIT_HISTORY
        ) {
    ...
    render() {
        ...
        const ariaLabel = localizeMessage({
            id: "accessibility.sections.lhsNavigator",
            defaultMessage: "channel navigator region",
        });
            ...
            >
                ...
                <SidebarList
                    handleOpenMoreDirectChannelsModal={
                        this.handleOpenMoreDirectChannelsModal
                    }
